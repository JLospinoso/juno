...
public class ObservedLikelihoodDerivativesCalculator implements LikelihoodDerivativesCalculator<AlterSelectionEffect> {
    @Override
    public LikelihoodDerivatives calculate(List<LikelihoodDerivatives> completeDataDerivatives, List<AlterSelectionEffect> effects) {
        ...
        scoreCovariance = scoreCovariance.scalarMultiply(inverseSampleSize);
        scoreCovariance = scoreCovariance.subtract(score.outerProduct(score));
        ObjectMatrix observedInformation = completeInformation.subtract(scoreCovariance);
        return new CommonsLikelihoodDerivatives(score, observedInformation, effects, logLikelihood);
    }
}
